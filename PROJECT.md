...
**Status:** active project reset · **Last updated:** 2026-08-31
...
The receipt, payload distribution, and sealed snapshot transport are complete internal slices. The wrapper still has no production call site, and current resident product gates remain local/LMDB-only. Product activation, multi-node service recovery, arbitrary crash-time consistency, recovery after loss of every sealed copy, trusted-node authorization and placement, the remaining system crash matrix, and multi-Region behavior remain open. Local development may exercise explicit authority through LMDB, but a local-only store cannot provide automatic failover after loss of its host. Other providers and a later peer-quorum store need their own certified profile without changing the public model.
