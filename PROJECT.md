...
**Status:** active project reset · **Last updated:** 2026-09-02
...
Local and single-node operation require no external Wharfie control plane. The distributed design fences explicit coordinator replacement with a linearizable durable authority record. Automatic replacement additionally requires a provider-certified renewal, failure-detection, epoch-transition, and transaction-fencing profile. The first certified profile is deliberately limited to single-Region, non-global DynamoDB. Its provider primitive, internal resident supervisor, deterministic validation, and disposable-table proof pass. Authority-bound history reconstruction, its retained admission/schedule-mutation quiescence barrier, provisioned replacement input, immutable execution-payload distribution, and a sealed LMDB application-state checkpoint transport are complete internal slices. Product activation and two-node recovery remain open. A later mesh milestone—not this provider profile—will validate worker and coordinator recovery across two nodes.
...
All enrolled nodes are trusted. The first distributed design has one coordinator that is authoritative at the durable-store boundary, not one irreplaceable coordinator machine. A partitioned process can continue believing it is leader and issuing messages, so correctness cannot depend on stopping stale processes. One durable authority record per application is co-located with its execution ledger. Initial acquisition and deliberate, caller-confirmed takeover are linearizable conditional transitions; takeover increments a monotonic epoch. The production operator path owns that confirmation, and control-store authoritative writers bind to the current tuple. New physical assignments carry its epoch, while each attempt additionally carries a per-invocation generation; storage rejects stale fencing values.

The repository now binds the production resident, direct durable-submission fallback, foreground execution, and direct mutating operator paths to the explicit authority state machine and execution-ledger transaction fence. Resident schedule-control writes consume that exact ledger token, while application-state writes use a separate destination-local adoption barrier and ordered control-store readiness protocol. New authority-bound admissions retain the exact admitting authority as durable provenance without changing their logical epoch-zero event fence. The internal replacement wrapper now closes or adopts a retained app-scoped monotonic execution-ledger barrier before two-pass history reconstruction, proves settled application-state history, selects an exact retained or durably hydrated snapshot replica, and only then performs ordinary application-state readiness preparation. It reopens only the exact closed predecessor after transport, readiness, and strong authority checks. Fresh manual, workflow, successor, and schedule decisions carry its exact `OPEN` generation, while exact committed replays remain readable during a close. This completes the bounded, explicit operator-confirmed replacement path and an internal automatic-replacement cutover composition; it does not establish public automatic failover, atomic cross-store handoff, arbitrary crash-time consistency, recovery after loss of every sealed copy, or multi-node recovery.
...
The DynamoDB primitive and internal resident supervisor have deterministic and live disposable-table proof, including renewal through drain, observation-backed takeover, fail-closed authority loss, a delayed stale predecessor rejected after takeover, and a current successor mutation retained. Startup validates topology through the exact immutable DynamoDB data client, pins all later operations to the returned full table ARN and TableId, and matches a provisioning-supplied opaque `tableResourceId` before constructing authority. Managed credentials can refresh and retain access or fail closed, but they cannot silently redirect work to a same-named table; independently starting nodes must carry the same expected resource identity. Provisioning must still prevent table recreation or topology change after startup. [ADR 0038](docs/architecture/decisions/0038-authority-bound-replacement-reconstruction.md) adds complete verified history reconstruction, and [ADR 0039](docs/architecture/decisions/0039-retained-coordinator-quiescence-barrier.md) adds its retained same-table cutover. Missing barrier state is compatibility-open but fenced as exact absence; close, newer-authority adoption, and exact-authority reopen advance one permanent generation with stable request receipts. A scheduled prepared token carries the generation it observed, so close/reopen cannot admit it through an ABA. Failures before a retained reopen leave admissions closed, and a final strong authority assertion still gates the handler after reopen.

[ADR 0040](docs/architecture/decisions/0040-provisioned-replacement-input-and-payload-distribution.md) now retains the replacement scope in one strict, credential-free, content-addressed receipt. Its durable handoff artifact pins the application and current revision, the expected DynamoDB profile, adapter, Region, table name and `tableResourceId`, the payload store and distribution identity, and the exact normalized application-state destination. Paths, credentials, timers, account IDs, endpoints, and full ARNs remain independently configured rather than copied into the receipt. A local-first provider-neutral replicated payload store publishes and reads back verified immutable bytes before returning a ledger reference; a replacement fetches and hydrates only on true local absence, never to mask corruption. The reconstructed wrapper validates the receipt, ambient configuration, ledger, and exact payload-store object before topology or authority and reconstructs through that read-through boundary.

[ADR 0041](docs/architecture/decisions/0041-sealed-lmdb-application-state-snapshot-transport.md) advances the receipt to version 2 and adds the bounded internal application-state transport. Under settled exact application-state history and the exact durable `CLOSED` barrier, publication writes a marker and seals the whole physical LMDB store before reading its bounded `data.mdb`; every ordinary mutation in every namespace sharing that store then fails until exact selected retained activation removes the seal, while an unselected source remains sealed. The provider publication call is not trusted as evidence: exact immediate readback and a final authority-and-barrier-fenced central publication record are required. Replacement validates an exact retained copy or durably claims a truly absent store root, creates `lmdb` exclusively, and hard-links privately staged `data.mdb` before snapshot-scoped evidence makes hydration logically committed. A central create-or-exact-replay activation binds the transport, the exact current `CLOSED` replacement barrier and authority, a `wasr1` physical replica, and `RETAINED` or `HYDRATED` status before destination-local activation and ordinary readiness. LMDB roots should remain dedicated to one application even though the physical seal correctly fences all namespaces in a shared file.

The receipt, payload distribution, sealed snapshot transport, and [explicit successor-authority repair](docs/architecture/decisions/0042-successor-authority-partial-hydration-repair.md) for one stale incomplete hydration-recovery receipt are complete internal slices. The repair retains the old receipt unchanged, records one stable repair plus append-only exact-current authorizations, and remains resumable across another takeover without creating repair-of-repair. The wrapper still has no production call site, and current resident product gates remain local/LMDB-only. Product activation, multi-node service recovery, arbitrary crash-time consistency, recovery after loss of every sealed copy, trusted-node authorization and placement, the remaining system crash matrix, and multi-Region behavior remain open. Local development may exercise explicit authority through LMDB, but a local-only store cannot provide automatic failover after loss of its host. Other providers and a later peer-quorum store need their own certified profile without changing the public model.
