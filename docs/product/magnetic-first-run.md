...
> explicitly replaced the dead coordinator from an exact inspection, and ran
> the same command; the standalone executable resumed exactly where it left
> off.
...
`prepare greeting`. `SIGKILL` leaves its coordinator authority ACTIVE, so the
showcase first retains an exact inspection and executes the explicit confirmed
takeover-and-release. Only then does repeating the exact named command visibly
continue from the retained timer without preparing the greeting a second time.
A bare repeat never replaces the ACTIVE resident. A graceful `SIGINT` or
`SIGTERM` separately drains without cancelling durable work and prints the same
resume command without requiring takeover. The final result remains inspectable
...
$ WHARFIE_DATA_ROOT=<data-root> <artifact> wharfie coordinator inspect \
    --json > authority-inspection.json
✓ Retained exact ACTIVE coordinator inspection.
<operator confirms that the killed predecessor should be fenced>
$ WHARFIE_DATA_ROOT=<data-root> <artifact> wharfie coordinator takeover \
    --inspection-file authority-inspection.json \
    --coordinator-id <fresh-temporary-id> \
    --request-id <stable-request-id> \
    --confirm-authority-replacement
✓ Inspected coordinator fenced; temporary successor released.

$ <same named run command>
...
artifact creation, relocation, interruption, retained exact inspection,
explicit confirmed takeover-and-release, identical named-run resumption, and an
inspectable result. The operator step safely replaces authority; the retained
preparation and timer prove separately that durable work did not repeat. A
developer should reach that sequence in under two minutes on a warm development
machine.
...
- The interruption happens after one committed step and before the timer
  completes.
- After `SIGKILL`, a bare repeat cannot replace the ACTIVE coordinator. The demo
  retains its exact inspection and performs explicit confirmed
  takeover-and-release first.
- After that operator safety step, repeating the exact named foreground command
  resumes the run and does not repeat the committed step or original timer.
...
7. Retain the exact packaged coordinator inspection and confirm from operational
   evidence that the killed predecessor should be fenced.
8. Execute explicit confirmed takeover-and-release from that inspection.
9. Repeat the exact named run command with the same artifact, arguments, and
   data root.
10. Observe the greeting complete without the first step or original timer
    being recreated.
11. Read and verify the retained terminal result from a later process with the
...
`SIGKILL`, retains an exact packaged coordinator inspection, executes explicit
confirmed takeover-and-release, then repeats the exact
artifact/name/arguments/data-root command. It proves one physical preparation
attempt and the original timer, and finally runs
