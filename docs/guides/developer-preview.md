...
- `wharfie-wharfie-0.0.15.tgz` is the AWS-provider-free core builder.
...
This ordinary package path always creates an AWS-provider-free SEA. Installing the
companion does not change ordinary application artifacts. If the application
must expose the packaged deployment surface, build a self-deployable operator;
install both handoff artifacts first when that outer operator must support AWS:
...
Wharfie accepts only the exact matching companion and embeds it into the outer
self-deployable operator. Its private Linux application payload remains
AWS-provider-free. The finished executable remains relocatable and does not need
the builder's `node_modules`. Omitting the companion keeps the AWS SDK graph
out of both layers and permanently seals AWS operations unavailable while
leaving deployment help and Hetzner operations available. Adding a companion
beside that executable later cannot change its packaged capability decision.
