...
attachments, then binds successful host attestation to durable V3 service
health across READY finalization. A pure durable activation kernel now drives
the fixed runtime-identity, application-storage, control-storage,
...
fixed IPv4 IMDSv2 token flow, pins the commercial regional STS, DynamoDB, and
S3 endpoints with one-attempt SDK policy, cancels and drains active work and
S3 response bodies, and exposes only the identity, activation-authority, and
artifact-read adapters. One production
...
the current local fence, re-proves record-directory durability after process
recreation, and fences and drains admitted work on close. That local fence
relationship remains distinct from controller authorization. The controller
now publishes one complete V65 request at a stable DynamoDB key only while its
exact all-settled head remains current, starting every attempt with fresh
managed-artifact evidence. The host strongly reads that request first and the
current head last for selector resolution and every V66
claim/dispatch/settle/replay decision under an exact-key runtime IAM grant.
The first concrete guest-content adapter now reads only the request's explicit
S3 VersionId under `s3:GetObjectVersion`, verifies all managed headers and
bytes, and atomically publishes the immutable SEA at
...
- [Durable attempt logs checkpoint](llm/checkpoints/2026-07-27-v88-durable-attempt-logs.md) — the latest restart point for sink-settled component acknowledgements, fenced bounded append/replay, durable manual/workflow wiring, sensitive-data and no-reader boundaries, focused verification, and V89.
...
- [Verified durable run-history checkpoint](llm/checkpoints/2026-07-27-v85-verified-run-history.md) — the parent restart point for fail-closed portable snapshots, shared source/packaged run discovery, explicit redaction and non-authority, focused verification, cleanup, and next work.
- [Read-only Docker proof readiness checkpoint](llm/checkpoints/2026-07-27-v84-read-only-docker-proof-readiness.md) — the separate parent checkpoint for the point-in-time mutation-incapable local admission observer and the still-unrun approval-only Docker proof.
- [Bounded Linux SEA proof tooling checkpoint](llm/checkpoints/2026-07-27-v83-bounded-linux-sea-proof-tooling.md) — the preceding checkpoint for the pull-disabled, residue-bounded Docker driver, exact-Node verifier, and strict non-authoritative `whlp2` receipt.
- [Real Linux SEA proof checkpoint](llm/checkpoints/2026-07-26-v82-linux-sea-proof.md) — the preceding non-authoritative Linux/x64 build and startup observation from a disposable trusted runner.
- [Exact-source SEA publication checkpoint](llm/checkpoints/2026-07-26-v81-exact-source-sea-publication.md) — the preceding checkpoint for exact-commit source capture, same-generation build evidence, immutable publication, and the V80 zero-argument delivery.
- [Read-only retained-storage provider evidence checkpoint](llm/checkpoints/2026-07-26-v79-read-only-provider-evidence.md) — the earlier restart point for the expiring evidence-only contract, mutation-incapable double-read SSM/EC2 collector, strict non-authoritative receipt, verification, and portable-delivery boundary.
