 ...
 * @property {import('../lib/db/base.js').DBClient} [db] - DB client instance.
 * @property {import('../lib/queue/base.js').QueueClient} [queue] - Queue client instance.
 * @property {import('../lib/object-storage/base.js').ObjectStorageClient} [objectStorage] - Object storage client instance.
 */
...
/**
 * @param {string} adapter - adapter.
 * @returns {Promise<(options?: any) => import('../lib/db/base.js').DBClient>} - Result.
 */
async function loadDBFactory(adapter) {
  if (adapter === 'vanilla') {
    const mod = await import('../lib/db/adapters/vanilla.js');
    return mod.default;
  }
  if (adapter === 'dynamodb') {
    const mod = await import('../lib/db/adapters/dynamodb.js');
    return mod.default;
  }
  if (adapter === 'lmdb') {
    const mod = await import('../lib/db/adapters/lmdb.js');
    return mod.default;
  }
...
/**
 * @param {string} adapter - adapter.
 * @returns {Promise<(options?: any) => import('../lib/queue/base.js').QueueClient>} - Result.
 */
async function loadQueueFactory(adapter) {
  if (adapter === 'vanilla') {
    const mod = await import('../lib/queue/adapters/vanilla.js');
    return mod.default;
  }
  if (adapter === 'sqs') {
    const mod = await import('../lib/queue/adapters/sqs.js');
    return mod.default;
  }
  if (adapter === 'lmdb') {
    const mod = await import('../lib/queue/adapters/lmdb.js');
    return mod.default;
  }
...
/**
 * @param {string} adapter - adapter.
 * @returns {Promise<(options?: any) => import('../lib/object-storage/base.js').ObjectStorageClient>} - Result.
 */
async function loadObjectStorageFactory(adapter) {
  if (adapter === 'vanilla') {
    const mod = await import('../lib/object-storage/adapters/vanilla.js');
    return mod.default;
  }
  if (adapter === 's3') {
    const mod = await import('../lib/object-storage/adapters/s3.js');
    return mod.default;
  }
  if (adapter === 'r2') {
    const mod = await import('../lib/object-storage/adapters/r2.js');
    return mod.default;
  }
  if (adapter === 'b2') {
    const mod = await import('../lib/object-storage/adapters/b2.js');
    return mod.default;
  }
