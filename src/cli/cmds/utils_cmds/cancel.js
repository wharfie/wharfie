...
import { resolveOperationsTableName } from '../../../core/lib/config/db.js';
...
async function createDBClient() {
  ...
  if (adapterName === 'dynamodb') {
    const { default: createDynamoDB } =
      await import('../../../core/lib/db/adapters/dynamodb.js');
  ...
  if (adapterName === 'lmdb') {
    const { default: createLMDB } =
      await import('../../../core/lib/db/adapters/lmdb.js');
    return createLMDB({ path });
  }

  const { default: createVanillaDB } =
    await import('../../../core/lib/db/adapters/vanilla.js');
  return createVanillaDB({ path });
}

/**
 * Cancels operations for a given resource ID, operation ID, or operation type.
 * @param {import('../../../core/lib/db/tables/operations.js').OperationsTableClient} store
