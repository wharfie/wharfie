 ...
 */
async function importBuildSelfModule() {
  /** @type {any} */
  let imported;
  await jest.isolateModulesAsync(async () => {
    imported = await import(BUILD_SELF_IMPORT);
  });
  return imported;
}

/**
 * @param {any} packageLocalApp - Mock packageLocalApp implementation.
 * @returns {Promise<any>} - Result.
 */
async function importBuildSelfModuleWithPackageLocalApp(packageLocalApp) {
  /** @type {any} */
  let imported;
  await jest.isolateModulesAsync(async () => {
...
describe('wharfie build-self', () => {
  ...
  test('resolveBuildSourceRoot prefers the current workspace when Wharfie sources are present', async () => {
    ...
    try {
      const mod = await importBuildSelfModule();
      expect(mod.hasBuildSources(tmpRepo)).toBe(true);
  ...
  test('buildSelf packages the self-hosted app via packageLocalApp and renames the final artifact', async () => {
    ...
    try {
      ...
      const mod =
        await importBuildSelfModuleWithPackageLocalApp(packageLocalApp);
      await mod.buildSelf({
        platform: 'linux',
        arch: 'amd64',
        nodeVersion: '24.13.1',
