...
const intrinsic = (module.exports = {});
 ...
 * @returns {any} - The original string, in Base64 representation.
 */
...
/**
 * [The intrinsic function Fn::Cidr](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-cidr.html)
 * returns an array of CIDR address blocks.  The number of CIDR blocks returned
 * is dependent on the count parameter.
 * @static
 * @name cidr
 * @param {string} ipBlock - The user-specified CIDR address block to be split into smaller CIDR blocks.
 * @param {number} count - The number of CIDRs to generate. Valid range is between 1 and 256.
 * @param {number} cidrBits - The number of subnet bits for the CIDR. For example, specifying a value "8" for this parameter will create a CIDR with a mask of "/24".
 * @returns {any} -
 */
intrinsic.cidr = (ipBlock, count, cidrBits) => {
 ...
 * @returns {any} The value that is assigned to SecondLevelKey.
 */
 ...
 * specifying region().
 * @returns {any} The list of Availability Zones for the region.
 */
 ...
 * @param {any[]} pieces - The list of values you want combined.
 * The delimiter will occur between fragments only. It will not terminate the
 * final value.
 * @returns {any} The combined string.
 */
 ...
 * array.
 * @param {any[]} list - The list of objects to select from. This list must not
 * be null, nor can it have null entries.
 * @returns {any} The selected object.
 */
 ...
 * to dereference.
 * @returns {any} The physical ID of the resource or the value of the parameter.
 */
intrinsic.ref = (name) => {
  return { Ref: name };
};
 ...
 * @static
 * @name userData
 * @param {any[]} lines - an array of commands
 * @returns {any} the array of commands, joined by `\n` and Base 64 encoded
 */
 ...
 * @returns {any} the value of an output exported by another stack.
 */
...
/**
 * Uses [the intrinsic function Fn::Sub](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-sub.html)
 ...
 * @returns {any} a Fn::Sub object defining the ARN
 */
intrinsic.arn = (service, suffix) => {
  const replacement = { service, suffix };
  if (service === 's3')
    return intrinsic.sub(
      'arn:${AWS::Partition}:${service}:::${suffix}',
      replacement
    );
  else
    return intrinsic.sub(
      'arn:${AWS::Partition}:${service}:${AWS::Region}:${AWS::AccountId}:${suffix}',
      replacement
    );
};

/**
 * [The intrinsic function Fn::Transform](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-transform.html)
 * returns the processed template snippet to be included in the processed stack template.
 * @static
 * @name transform
 * @param {string} name - the name of the CloudFormation macro, e.g. `AWS::Include`
 * @param {object} parameters - an object where each key is the name of a
 * parameter that should be passed to the macro function, and each value is the
 * value of that parameter
 * @returns {any} the processed template snippet to be included in the processed stack template.
 */
