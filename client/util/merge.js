 ...
 * @ignore
 * @name merge
 * @param {object} _arguments - a CloudFormation template to merge with
...
module.exports = function (..._arguments) {
  ...
  for (const arg of _arguments) {
    if (arg.Metadata)
      Object.keys(arg.Metadata).forEach((key) => {
        if (names.Metadata.has(key)) {
          try {
            assert.deepEqual(template.Metadata[key], arg.Metadata[key]);
          } catch (err) {
    ...
    if (arg.Parameters)
      Object.keys(arg.Parameters).forEach((key) => {
        if (names.Parameters.has(key)) {
          try {
            assert.deepEqual(template.Parameters[key], arg.Parameters[key]);
          } catch (err) {
    ...
    if (arg.Rules)
      Object.keys(arg.Rules).forEach((key) => {
        if (names.Rules.has(key)) {
          try {
            assert.deepEqual(template.Rules[key], arg.Rules[key]);
          } catch (err) {
            throw new Error('Rules name used more than once: ' + key);
          }
        }
    ...
    if (arg.Mappings)
      Object.keys(arg.Mappings).forEach((key) => {
        if (names.Mappings.has(key)) {
          try {
            assert.deepEqual(template.Mappings[key], arg.Mappings[key]);
          } catch (err) {
            throw new Error('Mappings name used more than once: ' + key);
          }
        }
    ...
    if (arg.Conditions)
      Object.keys(arg.Conditions).forEach((key) => {
        if (names.Conditions.has(key)) {
          try {
            assert.deepEqual(template.Conditions[key], arg.Conditions[key]);
          } catch (err) {
    ...
    if (arg.Resources)
      Object.keys(arg.Resources).forEach((key) => {
        if (names.Resources.has(key)) {
          try {
            assert.deepEqual(template.Resources[key], arg.Resources[key]);
          } catch (err) {
    ...
    if (arg.Outputs)
      Object.keys(arg.Outputs).forEach((key) => {
        if (names.Outputs.has(key)) {
          try {
            assert.deepEqual(template.Outputs[key], arg.Outputs[key]);
          } catch (err) {
    ...
    if (arg.Transform) {
      ...
      const transforms =
        typeof arg.Transform === 'string' ? [arg.Transform] : arg.Transform;
