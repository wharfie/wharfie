...
const update = async (resourceType) => {
  ...
  displayInfo(`Updating wharfie ${resourceType}...`);
  ...
  displaySuccess(`Updated wharfie ${resourceType}`);
};
...
exports.handler = async function ({ resourceType }) {
  try {
    await update(resourceType);
  } catch (err) {
    displayFailure(err);
  }
};
