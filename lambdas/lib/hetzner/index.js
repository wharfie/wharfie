...
class HetznerCloud {
   ...
   */
  buildCloudInitForSystemd(exec) {
    return this._buildCloudInitForSystemd(exec);
  }

  /**
   * @param {Required<Pick<import('./typedefs.js').CreateAndRunServiceOptions,'exec'>>['exec']} exec - Structured service description.
   * @returns {string} Cloud-init YAML with systemd unit and optional bootstrap payload.
   * @private
   */
  _buildCloudInitForSystemd(exec) {
    ...
    const argStr = args
      .map((/** @type {string} */ a) => shellArg(String(a)))
      .join(' ');
   ...
   */
  mergeUserData(original, ours) {
    return this._mergeUserData(original, ours);
  }

  /**
   * @param {string|undefined|null} original - Existing YAML/parts from the user.
   * @param {string} ours - Our generated cloud-config YAML.
   * @returns {string} Merged multi-part cloud-init content.
   * @private
   */
